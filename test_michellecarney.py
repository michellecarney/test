...
def f06(string1, string2):
    if len(string1) > len(string2):
        ...
        print(string1 + " is longer than " + string2 + " by {} chars".format(difference))
        percentage = (len(string2)/len(string1))*100
        print("{} has only {} the number of chars of {}".format(string2, percentage, string1))
    elif len(string2) > len(string1):
        difference = len(string2) - len(string1)
        print(string2 + " is longer than " + string1 + " by {} chars".format(difference))
        percentage = int((len(string1)/len(string2))*100)
        print("{} has only {}'%' the number of chars of {}".format(string1, percentage, string2))
...
###############################################################################
def f09():
    list_of_multiples = list([i for i in range(500) if ((i % 3) == 0) or ((i % 5) == 0)])
...
def f11(args):
    if type(args) == type('string'):
        return float(args)
    elif type(args) == type(2.33333):
        return int(args)
    elif type(args) == type(2):
        return 0
...
##Totally misread the prompt: I am printing the user input to a file sorry! haha
def f12():
    user_input = 0
    ...
    with open('log_file.txt', 'a') as log_file:
        try:
            while user_input != 'done':
                user_input = float(input("Give input, please: "))
